...
<main class="night-sky relative overflow-hidden min-h-screen bg-neutral-50">
  ...
  <div class=" max-w-5xl mx-auto px-6 py-10">
    ...
    <div class="bg-white dark:bg-white/5 backdrop-blur-lg text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
      {{-- Gradient Cover --}}
      <div class="relative">
        ...
        <div class="flex flex-col md:flex-row gap-6 px-6 -mt-16 pb-6 items-end">
          ...
          <div class="flex-1 pt-4">
            <h1 class="text-3xl md:text-4xl font-bold text-white">{{ $author->name }}</h1>
            @if($author->position)
              <p class="text-cyan-300 font-medium mt-1">{{ $author->position }}</p>
            @endif
      ...
      <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-10">
        <div class="space-y-6 md:col-span-2">
          ...
          @if($author->birth_date || $author->death_date)
            <div class="flex items-center gap-4 p-4 bg-white/10 border border-white/10 rounded-xl mb-3">
              <span class="text-2xl inline-block bg-white/10 text-white w-12 h-12 rounded-full flex items-center justify-center border border-white/20">📅</span>
              <div>
                <span class="text-slate-300 text-sm">Амьдралын он</span>
          ...
          @if(!empty($author->biography))
            <div>
              ...
              <div class="prose prose-invert max-w-none text-slate-200 leading-relaxed whitespace-pre-line">
                {{ $author->biography }}
              </div>
            </div>
          @endif
            ...
            <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3">
                @foreach($notableWorks as $work)
                  <div class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition">
                    <span class="inline-block bg-blue-500/20 w-8 h-8 flex items-center justify-center rounded text-blue-200 font-bold">📙</span>
                    <span class="text-slate-100">{{ $work }}</span>
                  </div>
                @endforeach
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-2">
                @foreach($awards as $award)
                  <div class="flex items-center gap-3 p-3 bg-amber-400/10 rounded border border-amber-200/30">
                    <span class="text-amber-300">🥇</span>
                    <span class="text-slate-100">{{ $award }}</span>
                  </div>
                @endforeach
              </div>
            </div>
        ...
        <div class="flex flex-col space-y-6">
          ...
          @if($author->email || !empty($socialLinks))
            <div class="bg-white/5 border border-white/10 rounded-xl p-5">
              ...
              <div class="space-y-3">
                @if($author->email)
                  <a href="mailto:{{ $author->email }}" class="flex items-center gap-2 text-slate-200 hover:text-cyan-300 transition">
                    <span class="bg-white/10 w-8 h-8 rounded-lg flex items-center justify-center border border-white/20">✉️</span>
                    <span class="text-sm truncate">{{ $author->email }}</span>
                  </a>
                @endif
                @if(!empty($socialLinks['website']))
                  <a href="{{ $socialLinks['website'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 text-slate-200 hover:text-cyan-300 transition">
                    <span class="bg-white/10 w-8 h-8 rounded-lg flex items-center justify-center border border-white/20">🌐</span>
                    <span class="text-sm">Вэбсайт</span>
                  </a>
                @endif
                @if(!empty($socialLinks['facebook']))
                  <a href="{{ $socialLinks['facebook'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 text-slate-200 hover:text-blue-400 transition">
                ...
                @if(!empty($socialLinks['twitter']))
                  <a href="{{ $socialLinks['twitter'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 text-slate-200 hover:text-blue-400 transition">
                ...
                @if(!empty($socialLinks['instagram']))
                  <a href="{{ $socialLinks['instagram'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 text-slate-200 hover:text-pink-400 transition">
                ...
                @if(!empty($socialLinks['linkedin']))
                  <a href="{{ $socialLinks['linkedin'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 text-slate-200 hover:text-blue-400 transition">
          ...
          <div class="bg-white/5 border border-white/10 rounded-xl p-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="text-center p-3 bg-white/10 rounded-lg border border-white/10">
                 <p class="text-2xl font-bold text-cyan-300">{{ $author->notable_works_count }}</p>
                <p class="text-xs text-slate-300">Бүтээл</p>
              </div>
              <div class="text-center p-3 bg-white/10 rounded-lg border border-white/10">
                 <p class="text-2xl font-bold text-amber-300">{{ $author->awards_count }}</p>
                <p class="text-xs text-slate-300">Шагнал</p>
              </div>
            </div>
          </div>
        </div>
