...
<div class="max-w-7xl mx-auto px-6 py-10">
    ...
    <form method="GET" action="{{ route('authors.index') }}" class="mb-8 flex justify-center gap-3">
        <div class="relative">
            <input 
                ...
                class="w-64 px-4 py-2 rounded-full border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <svg class="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
            </svg>
        </div>
        <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition-colors">
            Хайх
        </button>
        @if(request('q'))
            <a href="{{ route('authors.index') }}" class="px-6 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-full transition-colors">
                Цэвэрлэх
            </a>
        @endif
    </form>
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 masonry-grid">
            @foreach($authors as $author)
                <a href="{{ route('authors.show', $author->slug) }}"
                   class="card-3d block rounded-2xl border border-gray-100 dark:border-slate-800 hover:-translate-y-0.5 transition {{ $loop->index % 5 == 0 ? 'sm:col-span-2 sm:row-span-2' : ($loop->index % 3 == 0 ? 'md:row-span-2' : '') }}">
                    <div class="card-3d__inner bg-white dark:bg-slate-900 h-full">
                        <div class="card-3d__media {{ $loop->index % 5 == 0 ? 'h-96' : ($loop->index % 3 == 0 ? 'h-80' : 'h-56') }}">
