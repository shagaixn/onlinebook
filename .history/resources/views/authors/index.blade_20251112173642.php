...
<div class="max-w-7xl mx-auto px-6 py-10">
    <h1 class="text-3xl font-bold mb-6 text-slate-900 dark:text-white">Зохиолчид</h1>
    ...
    @if($authors->count() === 0)
    <div class="p-8 text-center text-gray-500 dark:text-gray-300 bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800">
            Зохиолч олдсонгүй.
        </div>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($authors as $author)
                <a href="{{ route('authors.show', $author->slug) }}" class="block bg-dark-900 dark:bg-slate-900 rounded-2xl shadow hover:shadow-lg border border-gray-100 dark:border-slate-800 overflow-hidden hover:-translate-y-0.5 transition">
                    ...
                    @endif
                    <div class="p-4">
                        <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ $author->name }}</h2>
                        <p class="text-sm text-gray-600 dark:text-gray-300">{{ $author->nationality ?? 'Үндэс тодорхойгүй' }}</p>
                        <p class="mt-2 text-sm text-gray-700 dark:text-gray-200">{{ Str::limit($author->biography, 100) }}</p>
                    </div>
                </a>
            @endforeach
        </div>
