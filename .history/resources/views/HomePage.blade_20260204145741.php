...
<main class="night-sky min-h-[100svh] w-full relative">
  ...
  <section class="w-full min-h-[100svh] flex flex-col justify-center items-center px-4 relative z-10 overflow-hidden">
    ...
    <h1 class="flex justify-center mb-12" aria-label="Book Plus">
      <span class="inline-flex items-baseline gap-4 px-10 py-5 rounded-[3rem] bg-dark dark:bg-white/5 backdrop-blur-md shadow-2xl">
        <span class="text-5xl md:text-7xl font-black text-blue-600 leading-none">Book</span>
        <span class="text-4xl md:text-6xl font-semibold text-dark dark:text-white leading-none">Plus</span>
      </span>
    </h1>
    ...
    <div class="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
      @if($isAuthenticated)
        <a href="{{ route('book') }}" 
           class="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-medium transition-all duration-200 shadow-lg">
          Ном
        </a>
          ...
          <div class="featured-book-item opacity-0 translate-y-20" data-index="{{ $index }}">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center {{ $index % 2 === 0 ? '' : 'lg:grid-flow-dense' }}">
              ...
              <div class="{{ $index % 2 === 0 ? '' : 'lg:col-start-2' }} group">
                ...
                @else
                  <div class="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-2xl cursor-not-allowed opacity-75">
                    <img 
                      src="{{ $book->cover_image ? asset('storage/'.$book->cover_image) : asset('images/placeholder-book.png') }}" 
                      alt="{{ $book->title }}"
                      class="w-full h-full object-cover"
                    ...
                    <div class="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <div class="bg-white dark:bg-gray-800 px-6 py-3 rounded-lg">
                        <p class="text-gray-900 dark:text-white font-semibold">🔒 Нэвтрэх шаардлагатай</p>
                      </div>
                    </div>
                  </div>
                @endif
...
<script>
// Featured Books Scroll Reveal Animation
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.featured-book-item').forEach(item => {
