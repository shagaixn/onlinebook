...
<main class="night-sky min-h-[100svh] max-w-9xl mx-auto px-4 py-15">
  @if(session('success'))
    <div class="bg-green-500/20 border border-green-500 text-green-200 px-4 py-3 rounded mb-6 text-center">
        {{ session('success') }}
    </div>
  @endif
  <h1 class="text-3xl sm:text-4xl font-extrabold text-center mb-2 text-slate-900 dark:text-white">
    <span class="text-gradient drop-shadow-sm">Subscription</span> төлөвлөгөө
  </h1>
  <p class="text-center text-sm text-slate-600 dark:text-slate-300 mb-10">Өөрт тохирох багцыг сонгоно уу.</p>

  <section aria-label="Subscription төлөвлөгөө" class="max-w-6xl mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
    @forelse($plans as $plan)
      <article class="group rounded-2xl bg-white dark:bg-white/5 backdrop-blur border border-gray-200 dark:border-white/10 overflow-hidden shadow-sm hover:shadow-lg hover:border-cyan-400/40 transition p-6 flex flex-col">
        <header class="mb-4">
          <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ $plan->name }}</h2>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ $plan->interval }}</p>
        </header>
        <div class="text-2xl font-bold mb-4 text-slate-900 dark:text-white">
          @if($plan->price === 0)
            <span class="text-green-600 dark:text-green-400">Free</span>
          @else
            <span class="text-cyan-600 dark:text-cyan-300">{{ number_format($plan->price) }}₮</span>
            <span class="text-xs font-medium text-slate-500 dark:text-slate-400">/{{ $plan->interval }}</span>
          @endif
        </div>
        <ul class="text-xs flex-1 space-y-2 mb-6">
          @foreach($plan->features as $f)
            <li class="flex items-start gap-2 text-slate-700 dark:text-slate-200">
              <svg class="w-4 h-4 text-cyan-500 dark:text-cyan-400 mt-0.5" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
        ...
        @auth
          <form method="POST" action="{{ route('subscription.pay') }}" class="mt-auto">
            @csrf
