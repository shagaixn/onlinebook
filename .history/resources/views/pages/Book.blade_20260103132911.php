        ...
        <section class="mb-10">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            @foreach($items as $book)
              <div class="group relative text-[13px]" data-book-id="{{ $book->id }}">
                <a href="{{ route('books.show', $book->id) }}" class="block focus:outline-none">
                  <div class="relative aspect-[2/3] rounded-lg overflow-hidden mb-2.5 shadow-md transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-0.5">
                    <img
                  ...
                  <div>
                    <h3 class="font-bold text-slate-900 dark:text-white text-[13px] leading-tight mb-0.5 line-clamp-1 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                      {{ $book->title }}
                    </h3>
                    <p class="text-slate-500 dark:text-slate-400 text-[11px] mb-0.5 line-clamp-1">
        ...
        @forelse ($books as $book)
          ...
          <div class="group relative text-[13px]" data-book-id="{{ $book->id }}">
            <a href="{{ route('books.show', $book->id) }}" class="block focus:outline-none">
              <div class="relative aspect-[2/3] rounded-lg overflow-hidden mb-2.5 shadow-md transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-0.5">
                @if($isNew)
                  <div class="absolute top-0 left-0 bg-red-500 text-white text-[9px] font-bold px-2 py-1 rounded-br-md z-10 uppercase tracking-wider">
                    Latest 24hours
                  </div>
                @else
                  <div class="absolute top-0 left-0 bg-slate-800/80 backdrop-blur text-white text-[9px] font-bold px-2 py-1 rounded-br-md z-10 uppercase tracking-wider">
              ...
              <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-[11px] font-medium mb-1 line-clamp-1">
                  {{ $book->author_display ?? 'Unknown' }}
                </p>
