...
<head>
  ...
  <script id="theme-init">
    (function () {
      try {
        // per-page server-side flag: if $forceTheme is passed from a view, it will be serialized below
        const FORCE_THEME_FLAG = typeof window !== 'undefined' && window.__FORCE_THEME_FLAG__ ? true : false;
        // NOTE: If a view sets a PHP variable $forceTheme, we will inject it below into window.__FORCE_THEME_FLAG__
        ...
        // If server-side requests this specific page to be dark, honor it.
        try {
          if (window.__FORCE_THEME_FLAG__ === true) {
            useDark = true;
          }
        } catch (e) { /* ignore */ }
        if (useDark) {
  ...
  <header class="border-b border-gray-100 dark:border-slate-800 px-0 pt-4 pb-2">
    <div class="max-w-7xl mx-auto flex items-center justify-between px-8">
      ...
      <div class="flex items-center gap-4">
        ...
        @auth
          <a href="{{ route('profile') }}" class="text-blue-600 font-semibold flex items-center gap-2">
