      ...
      <div class="relative">
        <div class="flex items-center justify-between gap-4 rounded-full border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 shadow-lg backdrop-blur px-4 sm:px-6 py-2.5">
          ...
          <div class="flex items-center gap-3 flex-shrink-0">
            <form class="relative hidden lg:block" action="{{ route('book') }}" method="GET">
              <input
                type="text"
                name="q"
                value="{{ request('q') }}"
                class="search-input h-10 w-64"
        ...
        <div id="mobile-menu" class="hidden absolute left-0 right-0 mt-3 md:hidden">
          <div class="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl p-4 space-y-4">
            <form class="relative" action="{{ route('book') }}" method="GET">
              <input
                type="text"
                name="q"
                value="{{ request('q') }}"
                class="search-input h-11 w-full"
                placeholder="Номын нэрээр хайх..."
                aria-label="Ном хайх"
              >
              <button type="submit" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-400 hover:text-blue-500 transition-colors" aria-label="Хайх">
            ...
            <div class="grid grid-cols-2 gap-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
              <a href="{{ route('home') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('home') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Танд зориулав</a>
              <a href="{{ route('book') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('book') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Ном</a>
              <a href="{{ route('subscription') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('subscription') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Subscription</a>
              <a href="{{ route('authors.index') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('authors.index') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Зохиолчид</a>
              <a href="{{ route('podcast') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('podcast') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Podcast</a>
              <a href="{{ route('manga') }}" class="px-3 py-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-800 dark:hover:text-blue-400 {{ request()->routeIs('manga') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-slate-800' : '' }}">Manga</a>
            </div>

            <div class="flex items-center justify-between rounded-2xl border border-slate-200 dark:border-slate-800 px-3 py-2">
            ...
            <div class="flex flex-col gap-2">
              @auth
                <a href="{{ url('/profile') }}" class="inline-flex items-center justify-between rounded-2xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">
                  <span class="font-semibold">{{ auth()->user()->name ?? 'Профайл' }}</span>
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </a>
              @else
                <a href="{{ route('login') }}" class="inline-flex items-center justify-between rounded-2xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">
