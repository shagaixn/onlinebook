...
<div class="max-w-4xl mx-auto mt-16 bg-white dark:bg-slate-900 p-10 rounded-2xl shadow-2xl transition-all duration-300">
    <h2 class="text-4xl font-extrabold mb-10 text-center text-gray-800 dark:text-gray-100">
    ...
    @auth
        ...
        <div class="space-y-8">
            <div class="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Нэр</label>
            ...
            <div class="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-user-pen text-blue-500"></i> Танилцуулга
                </h3>
                <p class="text-gray-700 dark:text-gray-300 leading-relaxed">
                    {{ Auth::user()->bio ?? 'Хэрэглэгч өөрийн талаар мэдээлэл оруулаагүй байна.' }}
                </p>
            </div>

            <div class="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-heart text-red-500"></i> Сонирхол ба хобби
                </h3>
                <ul class="list-disc list-inside text-gray-700 dark:text-gray-300">
                    @if(!empty(Auth::user()->interests))
                        @foreach(explode(',', Auth::user()->interests) as $interest)
                            <li>{{ trim($interest) }}</li>
                        @endforeach
                    @else
                        <li>Сонирхолын мэдээлэл оруулаагүй байна.</li>
                    @endif
                </ul>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-slate-800 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-database text-green-500"></i> Системийн мэдээлэл
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Үүрэг</label>
                        <p class="text-gray-800 dark:text-gray-100 font-medium">{{ Auth::user()->role ?? 'Хэрэглэгч' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Бүртгүүлсэн огноо</label>
                        <p class="text-gray-800 dark:text-gray-100 font-medium">{{ Auth::user()->created_at->format('Y-m-d') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-center flex-wrap gap-4">
                <a href="{{ route('profile.edit') }}" 
                   class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg shadow transition-transform hover:-translate-y-0.5">
                    ✏️ Засах
                </a>
                <button id="changePasswordBtn"
                        class="bg-gray-700 hover:bg-gray-800 text-white px-6 py-2.5 rounded-lg shadow transition-transform hover:-translate-y-0.5">
                    🔒 Нууц үг солих
                </button>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" 
                            class="bg-red-600 hover:bg-red-700 text-white px-6 py-2.5 rounded-lg shadow transition-transform hover:-translate-y-0.5">
                        🚪 Гарах
                    </button>
                </form>
            </div>
    ...
    @else
        <div class="text-center py-10">
            ...
            <a href="{{ route('login') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg shadow-md transition-transform hover:-translate-y-0.5">
                Нэвтрэх
            </a>
        </div>
    @endauth
