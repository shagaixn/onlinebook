            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-dark dark:bg-slate-800/50 backdrop-blur-xl border border-gray-200 dark:border-slate-700 rounded-2xl p-6 text-center relative overflow-hidden group shadow-sm dark:shadow-none">
                        ...
                        <div class="grid grid-cols-2 gap-2 border-t border-gray-200 dark:border-slate-700 pt-4">
                            ...
                            <div class="text-center border-l border-gray-200 dark:border-slate-700">
                                <div class="text-lg font-bold text-slate-900 dark:text-white">{{ $readingHistory->count() }}</div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div id="reading-history" class="bg-dark dark:bg-slate-800/50 backdrop-blur-xl border border-gray-200 dark:border-slate-700 rounded-2xl p-6 scroll-mt-24">
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2 border-b border-gray-200 dark:border-slate-700 pb-4">
                            <svg class="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                            </svg>
                            Уншсан номууд
                            <span class="ml-auto px-3 py-1 bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded-full text-sm font-semibold">
                        ...
                        @if($readingHistory->count() > 0)
                            <div class="space-y-4">
                                @foreach($readingHistory as $progress)
                                    @php
                                        $book = $progress->book;
                                        $percentage = round($progress->percentage ?? 0);
                                    @endphp
                                    <div class="flex gap-4 p-4 rounded-xl bg-dark-50 dark:bg-slate-900/30 border border-gray-200 dark:border-slate-700/50 hover:border-blue-400 dark:hover:border-blue-500 transition-all group">
                                        <a href="{{ route('books.show', $book->id) }}" class="flex-shrink-0">
                                            <img src="{{ $book->cover_image ? asset('storage/'.$book->cover_image) : asset('images/placeholder-book.png') }}" 
                                                 alt="{{ $book->title }}" 
                                                 class="w-20 h-28 object-cover rounded-lg shadow-md group-hover:shadow-xl transition-shadow">
                                        </a>
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('books.show', $book->id) }}" class="block">
                                                <h4 class="font-semibold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
                                                    {{ $book->title }}
                                                </h4>
                                            </a>
                                            <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                                {{ $book->author_display ?? 'Зохиолч байхгүй' }}
                                            </p>
                                            
                                            <!-- Progress Bar -->
                                            <div class="mt-3">
                                                <div class="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 mb-1">
                                                    <span>Уншсан хэмжээ</span>
                                            ...
                                            <div class="flex items-center gap-3 mt-3">
                                                <a href="{{ route('books.read', $book->id) }}" 
                                                   class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg font-medium transition-colors">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <div class="text-center py-12">
                                ...
                                <h4 class="text-lg font-semibold text-slate-900 dark:text-white mb-2">Уншсан ном байхгүй</h4>
                                <p class="text-slate-500 dark:text-slate-400 mb-6">Та номоо уншиж эхэлбэл энд харагдана</p>
                                <a href="{{ route('book') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                                    </svg>
