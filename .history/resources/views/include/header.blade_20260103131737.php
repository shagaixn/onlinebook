      ...
      <div class="relative">
        <div class="flex items-center justify-between gap-4 rounded-full border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 shadow-lg backdrop-blur px-4 sm:px-6 py-2.5">
          ...
          <div class="flex items-center gap-3 flex-shrink-0">
            <button data-search-toggle aria-label="Хайлт нээх" aria-expanded="false" class="inline-flex items-center justify-center w-11 h-11 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm text-slate-700 dark:text-slate-100 hover:text-blue-600 dark:hover:text-blue-400">
        ...
        <div id="search-panel" class="hidden absolute left-0 right-0 mt-3 z-40">
          <div class="max-w-screen-xl mx-auto px-4 sm:px-6">
            <form class="relative rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl overflow-hidden" action="{{ route('book') }}" method="GET">
              <input
                type="text"
                name="q"
                value="{{ request('q') }}"
                class="search-input h-12 w-full border-0 rounded-none bg-transparent pl-12 pr-4"
                placeholder="Номын нэрээр хайх..."
                aria-label="Ном хайх"
              >
              <button type="submit" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 transition-colors" aria-label="Хайх">
                <i class="fa-solid fa-magnifying-glass"></i>
              </button>
              <button type="button" data-search-close class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700 dark:text-slate-300 dark:hover:text-white" aria-label="Хайлт хаах">
  ...
  <script>
    (() => {
      ...
      const setState = (state) => {
        ...
        if (state && searchPanel) searchPanel.classList.add('hidden');
      };
    ...
    (() => {
      ...
      const searchInput = panel?.querySelector('input[name="q"]');
      ...
      const setState = (state) => {
        ...
        panel.classList.toggle('hidden', !open);
        toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
        if (state && menuPanel) menuPanel.classList.add('hidden');
        if (state && searchInput) setTimeout(() => searchInput.focus(), 0);
        if (!state && searchInput) searchInput.value = '';
      };
