...
<main class="night-sky min-h-[100svh] max-w-9xl mx-auto px-4 py-15">
  ...
  <section class="max-w-5xl mx-auto px-6 pt-20 pb-12 text-center relative z-10" aria-labelledby="hero-heading">
    ...
    <p class="mt-6 text-lg text-slate-300 max-w-2xl mx-auto leading-relaxed">
      ...
      <strong class="font-bold text-white bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-400">Book Plus</strong>-т тавтай морилно уу!
    </p>
  ...
  @php
    ...
    if (empty($marqueeItems)) {
      try {
        $marqueeItems = \App\Models\BookCategory::inRandomOrder()
          ->limit(15)
          ->pluck('name')
          ->map(fn ($name) => '🏷️ ' . $name)
          ->toArray();
  ...
  <!-- Marquee -->
  <section id="testimonials" class="mt-24 mb-12">
    <div class="relative w-full overflow-hidden bg-white/5 backdrop-blur-md border-y border-white/10">
      <div class="marquee-container">
        <div class="animate-marquee whitespace-nowrap py-6 px-2" aria-label="Wishlist дахь номуудыг гүйлгэн үзүүлж байна">
          @foreach($marqueeItems as $label)
            <span class="mx-8 text-xl sm:text-2xl text-white/90 font-semibold drop-shadow-md">{{ $label }}</span>
          @endforeach
          @foreach($marqueeItems as $label)
            <span class="mx-8 text-xl sm:text-2xl text-white/90 font-semibold drop-shadow-md" aria-hidden="true">{{ $label }}</span>
          @endforeach
        </div>
      </div>
      <div aria-hidden="true" class="pointer-events-none absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-slate-900 to-transparent"></div>
      <div aria-hidden="true" class="pointer-events-none absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-slate-900 to-transparent"></div>
    </div>
  </section>
</main>
<script>
  (function () {
    ...
    document.querySelectorAll('.wishlist-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.preventDefault(); // Prevent default if inside a link or form
        ...
        try {
          ...
          // Update classes for new design
          if (active) {
            btn.classList.remove('bg-black/30', 'border-white/20', 'hover:bg-pink-600', 'hover:border-pink-500');
            btn.classList.add('bg-pink-600', 'border-pink-500', 'shadow-lg', 'shadow-pink-600/30');
          } else {
            btn.classList.add('bg-black/30', 'border-white/20', 'hover:bg-pink-600', 'hover:border-pink-500');
            btn.classList.remove('bg-pink-600', 'border-pink-500', 'shadow-lg', 'shadow-pink-600/30');
          }
