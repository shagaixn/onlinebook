...
<main class="night-sky min-h-[100svh] max-w-9xl mx-auto px-4 py-15">
  ...
  <section class="max-w-4xl mx-auto px-6 pt-32 pb-24 text-center relative z-10">
    ...
    <div class="relative w-full py-10 my-8">
      {{-- Canvas Background --}}
      <canvas id="authors-canvas" class="absolute inset-0 w-full h-full bg-transparent rounded-2xl pointer-events-none"></canvas>
      
      {{-- HTML Grid (Foreground) --}}
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 relative z-10" id="authors-grid">
        @forelse($featuredAuthors as $author)
          <a href="{{ route('authors.show', $author->slug ?? $author->id) }}"
             class="group text-center transform-gpu will-change-transform transition-all duration-300 ease-out p-4 rounded-2xl hover:bg-white/5 backdrop-blur-sm"
             data-author-card
          >
            <div class="w-20 h-20 mx-auto rounded-full overflow-hidden border-2 border-gray-200 dark:border-white/30 mb-3 group-hover:border-cyan-400 dark:group-hover:border-cyan-400 transition-colors shadow-lg relative z-20 bg-white dark:bg-gray-800">
              <img
            ...
            <h3 class="text-gray-900 dark:text-white font-medium text-sm mb-1 group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors">
              {{ $author->name }}
            </h3>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class Particle {
        constructor() {
            ...
            this.baseColor = 'rgba(100, 200, 255, '; // Cyan-ish
        }
