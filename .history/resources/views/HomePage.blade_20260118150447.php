...
<script type="application/ld+json">
  @json($websiteJson, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)
</script>
...
<style>
  ...
  .book-hover { position: relative; overflow: hidden; }
  .book-hover::after {
    content: '';
  ...
  .book-hover:hover::after { width: 300px; height: 300px; }
</style>
...
<main class="night-sky min-h-[100svh] max-w-9xl mx-auto px-4 py-15">
  ...
  <section class="max-w-6xl mx-auto px-6 mt-24" role="region" aria-labelledby="featured-books-heading">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6" role="list">
      @forelse(($topRatedBooks ?? collect())->take(6) as $index => $book)
        @php
          ...
          $bookJson = [
            ...
            'author' => [
              '@type' => 'Person',
              'name' => $book->author ?? ($book->authorModel?->name ?? 'Unknown'),
            ],
            'aggregateRating' => [
              '@type' => 'AggregateRating',
              'ratingValue' => (float)($book->reviews_avg_rating ?? 0),
        ...
        <div class="book-card book-hover" data-book-id="{{ $book->id }}" role="listitem">
          ...
          <script type="application/ld+json">
            @json($bookJson, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)
          </script>
