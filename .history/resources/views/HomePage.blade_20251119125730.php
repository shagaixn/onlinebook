...
<style>
  ...
  /* Night sky background (gradient + soft glow) */
  .page-bg {
    background:
      radial-gradient(1200px 400px at 50% -10%, rgba(40,80,160,0.25), transparent 70%),
      linear-gradient(180deg, #050814, #0a1a3a 60%, #081129);
    position: relative;
    overflow: hidden;
    z-index: 0; /* canvas layers use negative z-index */
  }
  ...
  /* Layering: nebula at back, stars above nebula, then blur decorations */
  .sky-nebula { z-index: -30; opacity: 0.9; }
  .sky-stars  { z-index: -20; }

  /* Decorative background blobs stay above canvases but behind content */
  .bg-decor { z-index: -10; }
  ...
  /* NOTE: Старуудын хуучин ::before/::after давхаргыг устгасан */
</style>
