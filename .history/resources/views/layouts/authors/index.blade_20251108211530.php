    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
            <form action="{{ route('admin.authors.index') }}" method="GET" class="flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Нэр, имэйл эсвэл slug хайх"
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-600">Имэйл / Утас</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-600">Нийтлэгдсэн</th>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="font-semibold text-gray-800 dark:text-gray-100">
                                    <a href="{{ route('admin.authors.show', $author->id) }}">{{ $author->name }}</a>
                                </div>
                                <div class="text-xs text-gray-500">{{ $author->position ?? '' }}</div>
                                <div class="text-xs text-gray-400 mt-1">slug: <span class="text-sm text-gray-600">{{ $author->slug }}</span></div>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                                <div>{{ $author->email ?? '-' }}</div>
                                <div class="text-xs text-gray-500">{{ $author->phone ?? '-' }}</div>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                                {{ $author->created_at ? $author->created_at->format('Y-m-d') : '-' }}
                            </td>
                            <td class="px-4 py-3 text-right text-sm">
                                <div class="inline-flex items-center gap-2">
                                    <a href="{{ route('admin.authors.edit', $author->id) }}" class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">Засах</a>

                                    <form action="{{ route('admin.authors.destroy', $author->id) }}" method="POST" onsubmit="return confirm('Устгахдаа итгэлтэй байна уу?');">
                                        @csrf
                                        @method('DELETE')
