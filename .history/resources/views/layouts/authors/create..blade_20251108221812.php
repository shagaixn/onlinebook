...
<div class="max-w-3xl mx-auto p-6">
    ...
    <form action="{{ route('admin.authors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 bg-white dark:bg-slate-800 p-6 rounded-lg shadow">
        ...
        <div>
            ...
            <input name="slug" value="{{ old('slug') }}" maxlength="255"
                   class="mt-1 block w-full border rounded px-3 py-2 @error('slug') border-red-500 @enderror"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="birth_date" value="{{ old('birth_date') }}"
                       class="mt-1 block w-full border rounded px-3 py-2 @error('birth_date') border-red-500 @enderror">
            ...
            <div>
                ...
                <input name="birth_place" value="{{ old('birth_place') }}"
                       class="mt-1 block w-full border rounded px-3 py-2 @error('birth_place') border-red-500 @enderror">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="email" value="{{ old('email') }}" class="mt-1 block w-full border rounded px-3 py-2 @error('email') border-red-500 @enderror">
                @error('email') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium">Утас</label>
                <input name="phone" value="{{ old('phone') }}" class="mt-1 block w-full border rounded px-3 py-2 @error('phone') border-red-500 @enderror">
