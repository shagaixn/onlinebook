...
<div class="max-w-3xl mx-auto mt-12 bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-md">
    ...
    @endif
    <form action="{{ route('admin.authors.update', $author->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="social_links" value="{{ old('social_links', is_array($author->social_links) ? json_encode($author->social_links, JSON_UNESCAPED_UNICODE) : ($author->social_links ?? '')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder='{"twitter":"@user","website":"https://..."}'>
            ...
            <div>
                ...
                <input type="text" name="meta" value="{{ old('meta', is_array($author->meta) ? json_encode($author->meta, JSON_UNESCAPED_UNICODE) : ($author->meta ?? '')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder='{"awards":3}'>
            </div>
        </div>
        <div>
