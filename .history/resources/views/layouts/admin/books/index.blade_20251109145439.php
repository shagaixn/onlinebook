...
<div class="p-6">
    ...
    <div class="mb-8 flex justify-start">
        <form action="{{ route('admin.books.index') }}" method="GET" class="flex gap-3 w-full max-w-md">
            <input 
                type="text" 
                name="search" 
                value="{{ request('search') }}" 
                placeholder="Book нэрээр хайх..." 
        ...
        <table class="min-w-full border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-gray-50">
                    <td class="px-4 py-2">
                        @php
                            $cover = $book->cover_image ? asset('storage/' . $book->cover_image) : 'https://via.placeholder.com/64x64?text=No+Image';
                        @endphp
                        <img src="{{ $cover }}" alt="cover" class="w-16 h-16 object-cover rounded shadow">
                    </td>
