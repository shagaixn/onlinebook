...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-2xl shadow">
  ...
  <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="mb-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
      <label class="block font-medium text-gray-600 mb-2">Одоогийн ангилалууд:</label>
      <div class="flex flex-wrap gap-2">
        @foreach($categories as $cat)
          <span class="inline-block px-3 py-1 bg-white border border-gray-300 rounded-full text-sm text-gray-700">
            {{ $cat->name }}
          </span>
        @endforeach
      </div>
