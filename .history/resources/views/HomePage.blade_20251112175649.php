...
<main class="relative min-h-[60vh] overflow-hidden">
  ...
  <section class="max-w-5xl mx-auto px-6 pt-16 pb-10 text-center">
    <h1 class="text-4xl sm:text-5xl font-extrabold leading-tight tracking-tight">
      <span class="text-gradient drop-shadow-sm">#Мэдрэмж</span>, Мэдлэгийг өнгөлнө.
    </h1>
    ...
    <div class="mt-8 flex items-center justify-center gap-3">
      <a href="{{ route('subscription') }}"
         class="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 px-8 py-3 text-white font-semibold shadow-lg shadow-blue-500/20 hover:from-blue-600 hover:to-indigo-700 transition">
        Subscription
        <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 12h14M13 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </a>
      <a href="{{ route('book') }}"
         class="inline-flex items-center gap-2 rounded-full px-6 py-3 text-slate-700 dark:text-slate-200 ring-1 ring-slate-200/70 dark:ring-slate-700/70 bg-white/70 dark:bg-slate-900/50 backdrop-blur hover:bg-slate-50 dark:hover:bg-slate-800 transition">
        Номуудыг үзэх
      </a>
    </div>
  </section>
      ...
      @foreach($circleBooks as $i => $book)
        ...
        <img
          ...
          class="absolute {{ $pos['top'] ?? '' }} {{ $pos['bottom'] ?? '' }} {{ $pos['left'] ?? '' }} {{ $pos['right'] ?? '' }}
                 w-20 h-20 sm:w-24 sm:h-24 rounded-xl object-cover shadow-lg ring-1 ring-black/5 float-slow"
          style="animation-delay: {{ $i * 0.8 }}s"
        />
      @endforeach
    @else
      <img src="{{ asset('images/placeholder-book.png') }}" alt="Book 1" class="absolute top-4 left-6 w-20 h-20 sm:w-24 sm:h-24 rounded-xl object-cover shadow-lg ring-1 ring-black/5 float-slow" />
      <img src="{{ asset('images/placeholder-book.png') }}" alt="Book 2" class="absolute top-8 right-6 w-20 h-20 sm:w-24 sm:h-24 rounded-xl object-cover shadow-lg ring-1 ring-black/5 float-slow" style="animation-delay:.6s" />
      <img src="{{ asset('images/placeholder-book.png') }}" alt="Book 3" class="absolute bottom-6 left-1/2 -translate-x-1/2 w-20 h-20 sm:w-24 sm:h-24 rounded-xl object-cover shadow-lg ring-1 ring-black/5 float-slow" style="animation-delay:1.2s" />
    @endif
  ...
  <section class="w-full max-w-6xl mx-auto px-6 mt-24">
    <div class="flex items-end justify-between mb-6">
      <h2 class="text-2xl sm:text-3xl font-bold">Шинээр нэмэгдсэн номнууд</h2>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      @forelse($newBooks ?? [] as $book)
        <article class="group rounded-2xl bg-white/80 dark:bg-slate-900/60 backdrop-blur border border-slate-200/70 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition">
          <a href="{{ route('books.show', $book->id) }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60 rounded-2xl">
            @if($book->cover_image)
              <div class="aspect-[3/4] overflow-hidden">
                <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
              </div>
            @endif
            <div class="p-4">
  ...
  <section class="w-full max-w-6xl mx-auto px-6 mt-20">
    <div class="flex items-end justify-between mb-6">
      <h2 class="text-2xl sm:text-3xl font-bold">Шинээр нэмэгдсэн зохиолчид</h2>
          ...
          <a href="{{ route('authors.show', $author->slug) }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60 rounded-2xl">
            @if($author->profile_image)
              <div class="aspect-[3/2] overflow-hidden">
                <img src="{{ asset('storage/' . $author->profile_image) }}" alt="{{ $author->name }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
              </div>
            @endif
            <div class="p-4">
              <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">
