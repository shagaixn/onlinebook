...
<main class="night-sky min-h-[100svh] max-w-9xl mx-auto px-4 py-16 animate-fade-in">
  ...
  <section class="max-w-6xl mx-auto px-6 mt-24">
    <h2 class="text-2xl font-bold text-slate-900 dark:text-white mb-8 text-center">Номын ангилалууд</h2>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
      @forelse($categories as $cat)
        <a 
          href="{{ route('book', ['category' => $cat->name]) }}" 
          class="bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 hover:border-cyan-400/50 rounded-xl p-4 text-center transition group focus:outline-none focus:ring-2 focus:ring-cyan-400 shadow-sm dark:shadow-none">
          <span class="block text-2xl mb-2 group-hover:scale-110 transition" aria-hidden="true">📚</span>
          <span class="text-slate-700 dark:text-slate-200 font-medium block">{{ $cat->name }}</span>
  ...
  <section class="max-w-6xl mx-auto px-6 mt-12 mb-24">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      @forelse($newBooks as $book)
        ...
        <article 
          class="group relative rounded-2xl bg-white dark:bg-white/5 backdrop-blur border border-slate-200 dark:border-white/10 overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-cyan-400/30 hover:shadow-[0_20px_60px_-20px_rgba(34,211,238,0.5)] shadow-sm dark:shadow-none"
          ...
          <div class="p-4 flex items-center justify-between border-t border-slate-100 dark:border-white/5">
            <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm">
              <span>{{ $book->category ??  ($book->categoryModel?->name ?? 'General') }}</span>
            </div>
            
            <button 
              class="wishlist-btn p-2 rounded-full hover:bg-slate-50 dark:hover:bg-white/10 transition focus:outline-none focus:ring-2 focus:ring-pink-400 {{ $inWishlist ? 'text-pink-500' : 'text-slate-400' }}" 
              data-book-id="{{ $book->id }}" 
