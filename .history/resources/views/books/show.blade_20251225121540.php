...
<main class=" bg-dark from-blue-50 to-dark dark:from-slate-950 dark:to-slate-900 min-h-screen py-12 transition-colors duration-300">
  ...
  <div class="max-w-4xl mx-auto mt-12">
      <h3 class="text-2xl font-bold text-white mb-6">Төстэй номууд</h3>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
          @foreach($relatedBooks as $rBook)
              <a href="{{ route('books.show', $rBook->id) }}" class="group block bg-white/5 backdrop-blur border border-white/10 rounded-xl overflow-hidden hover:-translate-y-1 transition duration-300">
                  <div class="aspect-[2/3] overflow-hidden">
                      <img src="{{ $rBook->cover_image ? asset('storage/'.$rBook->cover_image) : asset('images/placeholder-book.png') }}" 
                           alt="{{ $rBook->title }}" 
                           class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                  </div>
                  <div class="p-4">
                      <h4 class="text-white font-semibold truncate">{{ $rBook->title }}</h4>
                      <p class="text-sm text-gray-400">{{ $rBook->author ?? $rBook->authorModel?->name }}</p>
                  </div>
              </a>
          @endforeach
      </div>
