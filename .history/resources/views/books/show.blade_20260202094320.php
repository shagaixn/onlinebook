    ...
    <div class="bg-white dark:bg-white/5 backdrop-blur border border-gray-200 dark:border-white/10 rounded-2xl p-6 mb-6 transition-colors duration-300">
      ...
      @auth
        <div class="mb-8 p-6 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
          <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Таны үнэлгээ</h4>
          ...
          <form action="{{ route('reviews.store') }}" method="POST">
              ...
              <div class="flex gap-2 items-center">
                @for($i = 1; $i <= 5; $i++)
                  <label class="cursor-pointer">
                    <input type="radio" name="rating" value="{{ $i }}" class="hidden star-radio" required>
            ...
            <div class="mb-4">
              ...
              <textarea name="comment" rows="4" class="w-full px-4 py-2 border border-gray-300 dark:border-white/20 rounded-lg bg-white dark:bg-white/5 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors" placeholder="Энэ номын талаархи таны бодол...">{{ old('comment') }}</textarea>
            </div>
