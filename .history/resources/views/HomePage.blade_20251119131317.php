...
<style>
  ...
  /* Гүн шөнийн тэнгэр + маш сул дээд гэрэл */
  .page-bg {
    background:
      radial-gradient(1400px 600px at 50% -200px, rgba(70,110,200,0.18), transparent 70%),
      linear-gradient(180deg,#05070d 0%, #081528 40%, #0a1f35 75%, #082234 100%);
    position:relative;
    overflow:hidden;
    z-index:0;
    min-height:75vh;
  ...
  /* Blur декоратив бөмбөлгүүд (nebula-гаас ДЭЭР биш) */
  .bg-decor { z-index:-10; }

  /* Marquee, spin, float (хуучин хэвээр) */
...
<main class="page-bg relative overflow-hidden">
  ...
  <!-- Decorative blur blobs (optional) -->
  ...
  <section class="max-w-5xl mx-auto px-6 pt-16 pb-10 text-center" aria-labelledby="hero-heading">
    <h1 id="hero-heading" class="text-4xl sm:text-5xl font-extrabold leading-tight tracking-tight text-white dark:text-white">
    ...
    <p class="mt-4 text-slate-300 max-w-2xl mx-auto">
      Хил хязгаар, цаг хугацааг үл харгалзан бүгдэд нээлттэйгээр мэдрэмж, мэдлэгийг түгээх <strong class="font-semibold text-white">Mbook</strong>-т тавтай морилно уу!
    </p>
    <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3" role="group" aria-label="Үйлдлүүд">
      <a href="{{ route('subscription') }}"
         class="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-3 text-white font-semibold shadow-lg shadow-indigo-700/30 hover:from-blue-500 hover:to-indigo-500 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-indigo-500"
      ...
      <a href="{{ route('book') }}"
         class="inline-flex items-center gap-2 rounded-full px-6 py-3 text-slate-100 ring-1 ring-slate-300/40 bg-white/10 backdrop-blur-md hover:bg-white/20 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-cyan-500"
  ...
  <div class="relative mx-auto mt-6 w-full max-w-xl h-72 sm:h-80">
    <div class="absolute inset-0 flex items-center justify-center">
      <div class="relative">
        <div class="absolute inset-0 rounded-full border border-blue-400/20 w-48 h-48 animate-spin-slow"></div>
    ...
    <p class="absolute -bottom-8 left-1/2 -translate-x-1/2 mt-6 text-slate-400 flex items-center gap-2">
  ...
  <section class="w-full max-w-6xl mx-auto px-6 mt-24">
    <div class="flex items-end justify-between mb-6">
      ...
      @if(!empty($newBooks) && count($newBooks) > 0)
        <a href="{{ route('book') }}" class="text-cyan-400 hover:underline text-sm">Бүгдийг харах</a>
      @endif
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        <article class="group rounded-2xl bg-white/5 backdrop-blur border border-white/10 overflow-hidden shadow-sm hover:shadow-lg hover:border-cyan-400/40 transition">
          <a href="{{ route('books.show', $book->id) }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/70 rounded-2xl">
            ...
            <div class="p-4">
              ...
              <div class="mt-3 inline-flex items-center gap-1 text-cyan-400 text-sm">
                Дэлгэрэнгүй
      ...
      @empty
        <div class="col-span-1 sm:col-span-2 md:col-span-3 text-center text-slate-400">
          Шинэ ном алга байна.
        </div>
      @endforelse
    </div>
  ...
  <section class="w-full max-w-6xl mx-auto px-6 mt-20">
    <div class="flex items-end justify-between mb-6">
      ...
      @if(!empty($newAuthors) && count($newAuthors) > 0)
        <a href="{{ route('authors.index') }}" class="text-cyan-400 hover:underline text-sm">Бүгдийг харах</a>
      @endif
    </div>
  ...
  <section id="testimonials" class="mt-24">
    <div class="relative w-full overflow-hidden bg-[#061225]">
      ...
      <div aria-hidden="true" class="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-[#061225] to-transparent"></div>
      <div aria-hidden="true" class="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-[#061225] to-transparent"></div>
    </div>
  </section>
</main>
