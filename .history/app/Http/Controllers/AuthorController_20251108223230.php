...
class AuthorController extends Controller
{
    ...
    {
        // Try the expected view first
        if (View::exists('layouts.authors.create')) {
            return view('layouts.authors.create');
        }

        // Fallback to legacy or misnamed file if it exists (adjust path if needed)
        $alt = resource_path('views/layouts/author/create..blade.php');
        if (file_exists($alt)) {
            return view()->file($alt);
        }

        // Fail early with a helpful message so you can fix file name/path
        abort(404, 'Create view not found. Expected view: layouts.authors.create or file at: ' . $alt);
    }
    ...
    public function show($slug)
    {
        ...
        // Use the same view namespace as index/create/edit
