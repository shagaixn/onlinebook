...
class DashboardController extends Controller
{
    ...
    public function bookChart()
    {
        ...
        $mostReadCategories = \App\Models\BookCategory::select('book_categories.*')
            ->join('books', 'books.category_id', '=', 'book_categories.id')
            ->join('reading_progress', 'reading_progress.book_id', '=', 'books.id')
            ->groupBy('book_categories.id', 'book_categories.name', 'book_categories.slug', 'book_categories.created_at', 'book_categories.updated_at')
            ->orderBy(\DB::raw('COUNT(reading_progress.id)'), 'desc')
            ->get()
            ->map(function($category) {
                $readingCount = \App\Models\ReadingProgress::whereHas('book', function($query) use ($category) {
