...
class HomeController extends Controller
{
    ...
    public function authors(Request $request)
    {
        ...
        $searchType = $request->get('type', 'authors');
        ...
        if ($searchQuery) {
            ...
            if ($searchType === 'authors' || $searchType === 'both') {
                $authors = Author::where(function ($q) use ($searchQuery) {
                    $q->where('name', 'like', "%{$searchQuery}%")
                      ->orWhere('nationality', 'like', "%{$searchQuery}%")
                      ->orWhere('biography', 'like', "%{$searchQuery}%");
                })->orderBy('name')->paginate(12)->withQueryString();
            }
            
            // Search books
            if ($searchType === 'books' || $searchType === 'both') {
                $books = \App\Models\Book::where(function ($q) use ($searchQuery) {
                    $q->where('title', 'like', "%{$searchQuery}%")
                      ->orWhere('description', 'like', "%{$searchQuery}%")
                      ->orWhere('author', 'like', "%{$searchQuery}%");
                })->with('author')->orderBy('title')->paginate(12)->withQueryString();
            }
        } else {
            // Default: show all authors
            $authors = Author::orderBy('name')->paginate(12)->withQueryString();
        }

        return view('authors.index', compact('authors', 'books', 'searchType'));
    }
