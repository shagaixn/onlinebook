...
class AuthorController extends Controller
{
    public function index(Request $request)
    {
        ...
        // If request is from admin prefix and admin view exists, return admin view
        if ($request->is('admin/*') && View::exists('admin.authors.index')) {
            return view('admin.authors.index', compact('authors'));
        }

        // Prefer pages.authors if you created resources/views/pages/authors.blade.php
        if (View::exists('pages.authors')) {
            return view('pages.authors', compact('authors'));
        }

        // Fallback to layouts.authors.* namespace
        return view('layouts.authors.index', compact('authors'));
    }

    public function create()
    {
        // Try admin namespace first (common), then layouts.authors, then legacy file path
        if (View::exists('admin.authors.create')) {
            return view('admin.authors.create');
        }

        if (View::exists('layouts.authors.create')) {
            return view('layouts.authors.create');
        }

        $alt = resource_path('views/layouts/author/create..blade.php');
        if (file_exists($alt)) {
            return view()->file($alt);
        }

        abort(404, 'Create view not found. Expected one of: admin.authors.create, layouts.authors.create, or file at: ' . $alt);
    }
    ...
    public function show($slug)
    {
        ...
        // Prefer a dedicated pages view for show (resources/views/pages/authors/show.blade.php)
        if (View::exists('pages.authors.show')) {
            return view('pages.authors.show', compact('author'));
        }

        // If the single file resources/views/pages/authors.blade.php is designed to handle a $author (you created it that way)
        if (View::exists('pages.authors')) {
            return view('pages.authors', compact('author'));
        }

        // Fallback to layouts.authors.show
        return view('layouts.authors.show', compact('author'));
    }
    ...
    public function edit($id)
    {
        ...
        // Ensure we return the view that matches the layouts.authors.* namespace
        return view('layouts.authors.edit', compact('author'));
    }
