...
class AuthorController extends Controller
{
    ...
    {
        abort_if(!View::exists('layouts.admin.authors.create'), 404, 'Create view not found. Expected: layouts.admin.authors.create');
    ...
    public function store(Request $request)
    {
        ...
        $saveData = $this->filterAuthorColumns($saveData);

        Author::create($saveData);

        return redirect()->route('layouts.admin.authors.index')->with('success', 'Зохиолч амжилттай нэмэгдлээ.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $updateData = [
            ...
            'social_links' => !empty($data['social_links']) ? $data['social_links'] : $author->social_links,


        ];

        $updateData = $this->filterAuthorColumns($updateData);
    ...
    protected function filterAuthorColumns(array $attributes): array
    {
        static $authorColumns;

        if (!$authorColumns) {
            $authorColumns = Schema::getColumnListing('authors');
        }

        return collect($attributes)
            ->filter(fn ($value, $key) => in_array($key, $authorColumns, true))
            ->toArray();
    }
}