 ...
 * Location: resources/js/home.js
 * Include via Blade: @push('page-scripts') @vite('resources/js/home.js') @endpush
 * (Requires @stack('page-scripts') inside <head> and laravel-vite-plugin.)
 *
 * Improvements:
 * - Background image preload and 'bg-loaded' class toggle for smoother paint.
 * - Robust marquee controls (pause on hover/focus/keyboard + play/pause button).
 * - IntersectionObserver adds `.in-view` for entrance effects.
 * - Global image error fallback to placeholder.
 * - Prevents double form submissions and sets aria-busy.
 * - Link transition handling for a[data-transition].
